...
```ts
const dgDecisionApproved = formalDgReview ? String(formalDgReview.decision) === "approved" : false;
const meetingHeld = formalMeeting ? String(formalMeeting.status) === "held" : false;
const canClosePhase = !!(
  phase.formalRequestCourrierId && dgDecisionApproved && meetingHeld &&
...
```ts
closure: { recevabilityCourrierDocumentId, phaseClosureCourrierDocumentId, canClosePhase }
```
